    ...
    const clientServices = require('../services/clientServices');
        ...
        if (!username || !password) {
            ...
            if (request.session && request.session.user) {
                response.render("resultlog", {l : 'Already logged in'});
            } else {
                clientServices.loginService(username, password, function(err, dberr, client) {
                    ...
                    if (client === null) {
                        ...
                        response.render("resultlog", {l : 'Login failed'}); //invite to register
                    } else {
                        ...
                        response.render("resultlog", { l:`Login for ${username} with ID.${client[0].num_client} was successful!` });
                    }
                });
            }
        }
    ...
    const registerControl = (request, response) => {
        ...
        clientServices.registerService(client, function(err, exists, insertedID) {
            ...
            if (exists) {
                ...
                response.render("register", { l :`registration failed. Username (${username}) already taken!` ,}); //invite to register
            } else {
                client.num_client = insertedID;response.render("log", { l:`Login for ${username} with ID.${client[0].num_client} was successful!` ,});
                console.log(`Registration (${username}, ${insertedID}) successful!`);
                response.render("register", { l :`Successful registration ${client.contact} (ID.${client.num_client})!`});
            }
            response.end();
        });
    };
